        ...
        **kwargs,
        ...
        if train_params.init_model_dir:
            ...
            if prob.Y_label.shape[1] != matcher.nr_labels:
                LOGGER.warning(
                    f"Got mismatch nr_labels (expected {prob.Y_label.shape[1]} but got {matcher.nr_labels}), text_model reinitialized!"
                )
                matcher.text_model = TransformerLinearXMCHead(
                    matcher.text_encoder.config.hidden_size, prob.Y_label.shape[1]
                )
                matcher.text_encoder.config.num_labels = prob.Y_label.shape[1]
        else:
        ...
        if bootstrapping is not None:
            ...
            if "linear" in train_params.bootstrap_method:
                bootstrap_prob = MLProblem(
                    ...
                    M=prob.M,
                    R=prob.Y_label if "weighted" in train_params.bootstrap_method else None,
                )
        ...
        if train_concat:
            ...
            lprob = MLProblem(
                ...
                C=prob.C if prob.M is not None else None,
                M=prob.M,
                R=sk_normalize(prob.Y_label, norm="l1") if train_params.cost_sensitive_ranker else None,
            )
        ...
        if val_prob is not None:
            ...
            val_metrics = smat_util.MetricsMLabel.generate(val_prob.Y_label, P_val, topk=pred_params.only_topk)
            avr_val_beam = (
...
class TransformerMultiTask(pecos.BaseClass):
    """Transformer Multi-task Model

    Construct, fine-tune the transformers and:
    - Predict on a fixed output label space for multi-label classification XMC
    - Predict for multi-class classification
    """

    LOSS_FUNCTION_TYPES_MLABEL = {
        "hinge": HingeLoss(margin=1.0, power=1),
        "squared-hinge": HingeLoss(margin=1.0, power=2),
        "weighted-hinge": HingeLoss(margin=1.0, power=1, cost_weighted=True),
        "weighted-squared-hinge": HingeLoss(margin=1.0, power=2, cost_weighted=True),
    ...
    class TrainParams(pecos.BaseParams):  # type: ignore
        ...
        loss_function (str): type of loss function to use for transformer
            training. Default 'squared-hinge'
        bootstrap_method (str): algorithm to bootstrap text_model. If not None, initialize
            TransformerMultiTask projection layer with one of:
                'linear' (default): linear model trained on final embeddings of parent layer
                'inherit': inherit weights from parent labels
        lr_schedule (str): learning rate schedule. See transformers.SchedulerType for details.
            Default 'linear'
        ...
        cache_dir (str): dir to store the pre-trained models downloaded from
            s3. Default empty to use a temp dir.
        init_model_dir (str): path to load checkpoint of TransformerMultiTask. If given,
            start from the given checkpoint rather than downloading a
            pre-trained model from S3. Default empty to ignore
        """
        ...
        model_shortcut: str = "bert-base-cased"
        negative_sampling: str = "tfn"
        loss_function_mlabel: str = "squared-hinge"
        loss_function_mclass: str = "cross-entropy"
        bootstrap_method: str = "linear"
        lr_schedule: str = "linear"
    ...
    class PredParams(pecos.BaseParams):  # type: ignore
        ...
        post_processor (str, optional):  the post_processor specified in the model.
            Default to "noop"
        ensemble_method (str, optional): micro ensemble method to generate prediction.
            Default to "transformer-only". See TransformerMultiTask.ensemble_prediction for details.
        truncate_length (int, optional): length to truncate input text, default 128.
    ...
    def __init__(
        ...
        Args:
            ...
            concat_model (MLModel): linear model that takes concatenation of transformer
                embeddings and input numerical features to predict on label space
            train_params (TransformerMultiTask.TrainParams, optional): instance of TransformerMultiTask.TrainParams.
            pred_params (TransformerMultiTask.PredParams, optional): instance of TransformerMultiTask.PredParams.
            model_folder (tempfile.TemporaryDirectory): Temporary directory object. This is a workaround for certain
                tokenizer models that rely on Sentencepiece, which is not in memory.
        """
        self.text_encoder = text_encoder
    ...
    @classmethod
    def get_loss_function_mlabel(cls, loss_function):
        """Get the loss function for training

        Args:
            loss_function (str): type of loss function for multi-label classification,
            in TransformerMultiTask.LOSS_FUNCTION_TYPES_MLABEL

        Returns:
            loss_function (torch.nn.module)
        """
        return cls.LOSS_FUNCTION_TYPES_MLABEL[loss_function]

    @classmethod
    def get_loss_function_mclass(cls, loss_function):
        """Get the loss function for training

        Args:
            loss_function (str): type of loss function for multi-class classification,
            in TransformerMultiTask.LOSS_FUNCTION_TYPES_MCLASS

        Returns:
            loss_function (torch.nn.module)
        """
        return cls.LOSS_FUNCTION_TYPES_MCLASS[loss_function]
    ...
    def load(cls, load_dir, num_classes, mclass_pred_hyperparam=None, freeze_mclass_head=False):
        ...
        with open(os.path.join(encoder_dir, "config.json"), "r", encoding="utf-8") as fin:
            transformer_type = json.loads(fin.read())["model_type"] + "-multitask"
        dnn_type = ENCODER_CLASSES[transformer_type]
        encoder_config = dnn_type.config_class.from_pretrained(encoder_dir)
        text_encoder, loading_info = dnn_type.model_class.from_pretrained(
    ...
    def download_model(cls, model_shortcut, num_classes, num_labels=2,
        ...
        Args:
            model_shortcut (str): model name shortcut, e.g. 'bert-base-cased'
            num_labels (int): model output size for multi-label classification task
        ...
        if config.model_type + "-multitask" not in ENCODER_CLASSES:
            raise ValueError(f"Model type {config.model_type}-multitask not supported.")
    ...
    def predict(
        ...
        **kwargs,
        ...
        if pred_params is None:
            ...
            pred_params = self.get_pred_params().override_with_kwargs(pred_params)
        elif not isinstance(pred_params, TransformerMultiTask.PredParams):
            raise TypeError(f"Unsupported type for pred_params: {type(pred_params)}")
        ...
        if max_pred_chunk is None or max_pred_chunk >= nr_inst:
            label_pred, class_pred, embeddings = self._predict(
                X_text,
                X_feat=X_feat,
                Y_class=Y_class,
                csr_codes=csr_codes,
            ...
            if not all(pp is None for pp in P_label_chunks):
                label_pred = smat_util.vstack_csr(P_label_chunks)
                class_pred = np.concatenate(P_class_chunks, axis=0)
            else:
                # only_embeddings case
                label_pred = None
                class_pred = None
        ...
        **kwargs,
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                ...
                if not only_embeddings:
                    c_pred_label = outputs["logits_mlabel"]
                    c_pred_class = outputs["logits_mclass"]

                    batch_cpred_class.append(c_pred_class.cpu().numpy())

                    # get topk prediction
                    if csr_codes_next is None:  # take all labels into consideration
                        cpred_csr = smat.csr_matrix(c_pred_label.cpu().numpy())
                        cpred_csr.data = PostProcessor.get(pred_params.post_processor).transform(
                            cpred_csr.data, inplace=True
                        )
                        cpred_csr = smat_util.sorted_csr(cpred_csr, only_topk=local_topk)
                        batch_cpred_label.append(cpred_csr)
                    else:
                        cur_act_labels = csr_codes_next[inputs["instance_number"].cpu()]
                        nnz_of_insts = cur_act_labels.indptr[1:] - cur_act_labels.indptr[:-1]
                        inst_idx = np.repeat(
                        ...
                        label_idx = cur_act_labels.indices.astype(np.uint32)
                        val = c_pred_label.cpu().numpy().flatten()
                        val = val[
                            np.argwhere(
                                inputs["label_indices"].cpu().flatten() != self.text_model.label_pad
                            )
        ...
        pred_class = None
        if not only_embeddings:
            pred_class = np.concatenate(batch_cpred_class, axis=0)

            pred_csr_codes = smat_util.vstack_csr(batch_cpred_label)
            ens_method = pred_params.ensemble_method
            # concat_model prediction requires concat_model and X_feat
            if all(v is not None for v in [self.concat_model, X_feat]):
                concat_pred_csr_codes = self.concat_model.predict(
                    TransformerMultiTask.concat_features(X_feat, embeddings, normalize_emb=True),
                    csr_codes=csr_codes,  # use original csr_codes rather than csr_codes_next
                    only_topk=local_topk,
                    post_processor=pred_params.post_processor,
                )
                pred_csr_codes = TransformerMultiTask.ensemble_prediction(
                    pred_csr_codes, concat_pred_csr_codes, local_topk, ens_method
                )
            elif self.concat_model is not None and ens_method != "transformer-only":
                LOGGER.warning(
        ...
        return pred_csr_codes, pred_class, embeddings
    ...
    def fine_tune_encoder(self, prob, val_prob=None, val_csr_codes=None, finetune_round_th=None, mclass_weight=1):
        ...
        Args:
            prob (MLMultiTaskProblemWithText): training problem
            val_prob (MLMultiTaskProblemWithText, optional): validation problem
            val_csr_codes (csr_matrix, optional): prediction matrix for
                    validation data, shape = (nr_val_inst, nr_codes)
                    its values and indices will be used in combiner for next
                    prediction
        ...
        loss_function_mclass = TransformerMultiTask.get_loss_function_mclass(train_params.loss_function_mclass).to(
            self.device
        )
        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs:
                ...
                return_train_embeddings (bool): if True and return_dict, return training instance embeddings
        Returns:
            results (TransformerMultiTask or dict):
            if return_dict=True, return a dictionary:
                {
                    matcher: TransformerMultiTask instance
                    trn_pred (csr_matrix): topk prediction on training data, shape = (nr_inst, nr_labels)
                    val_pred (csr_matrix or None): topk prediction on validation data, shape = (nr_val_inst, nr_labels)
                    trn_embeddings (ndarray): instance embedding on training data, shape = (nr_inst, hidden_dim).
                    val_embeddings (ndarray or None): instance embedding on validation data, shape = (nr_val_inst, hidden_dim).
                }
            otherwise return the trained TransformerMultiTask instance
        """
        train_params = cls.TrainParams.from_dict(train_params)
        pred_params = cls.PredParams.from_dict(pred_params)
        ...
        P_label_trn, P_class_trn, inst_embeddings = None, None, None
        train_concat = pred_params.ensemble_method not in ["transformer-only"]
        if train_concat or return_train_pred or return_train_embeddings:
            # getting the instance embeddings of training data
            # since X_feat is not passed, transformer-only result is produced
            P_label_trn, P_class_trn, inst_embeddings = matcher.predict(
                prob.X_text,
        ...
        if train_concat:
            ...
            # P_label_trn with concat_model
            concat_P_label_trn = matcher.concat_model.predict(
                lprob.X,
                csr_codes=csr_codes,
                only_topk=pred_params.only_topk,
                post_processor=pred_params.post_processor,
            )
            P_label_trn = TransformerMultiTask.ensemble_prediction(
                P_label_trn,
                concat_P_label_trn,
                pred_params.only_topk,
                pred_params.ensemble_method,
        ...
        P_label_val, P_class_val, val_inst_embeddings = None, None, None
        if val_prob is not None:
            P_label_val, P_class_val, val_inst_embeddings = matcher.predict(
                val_prob.X_text,
                X_feat=val_prob.X_feat,
                csr_codes=val_csr_codes,
                Y_class=val_prob.Y_class,
            ...
            LOGGER.info(
                "| {} mlabel-test-prec {}".format(
                    val_type,
                    " ".join("{:4.2f}".format(100 * v) for v in val_metrics_mlabel.prec),
                )
            )
            LOGGER.info(
                "| {} mlabel-test-recl {}".format(
                    val_type,
                    " ".join("{:4.2f}".format(100 * v) for v in val_metrics_mlabel.recall),
                )
            )
            LOGGER.info(
                "| mclass-test-acc {:4.2f}".format(100 * val_metrics_mclass.acc),
        ...
        if return_dict:
            return {
                "matcher": matcher,
                "trn_pred_label": P_label_trn if return_train_pred else None,
                "trn_pred_class": P_class_trn if return_train_pred else None,
                "val_pred_label": P_label_val,
                "val_pred_class": P_class_val,
                "trn_embeddings": inst_embeddings if return_train_embeddings else None,
                "val_embeddings": val_inst_embeddings,
            }
        else:
